...
def convert_to_condition(
    ...
    if condition_type == "depth":
        ...
        return edges
    # Added handler for sketch_edit inference simulation
    elif condition_type == "sketch_edit":
        # For inference: if raw_img is a photo, we simulate a sketch using Canny
        # For training: this function is usually bypassed as we load real sketches
        img = np.array(raw_img.convert("L"))
        # Invert if necessary (Sketches should be black lines on white background usually)
        # But Canny gives white lines on black. Let's align to dataset standard.
        edges = cv2.Canny(img, 100, 200) 
        edges = Image.fromarray(edges).convert("RGB")
        return edges
    ...
    **params: dict,
    ...
    if kv_cache:
        ...
        for module in self.transformer.modules():
            if isinstance(module, Attention) or "FluxAttention" in module.__class__.__name__:
